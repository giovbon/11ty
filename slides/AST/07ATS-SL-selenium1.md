...
**PyAutoGUI** controla o sistema operacional inteiro de forma "cega", baseado no que está visível na tela. Clica em **coordenadas da tela** (ex: clique no pixel X: 500, Y: 300) ou procura por uma imagem específica que você salvou
...
**Selenium** controla exclusivamente navegadores web lendo o código por trás do site. Interage diretamente com o código fonte do site (o HTML). Em vez de dizer "clique no meio da tela", você diz ao Selenium: "encontre o botão que tem o ID 'botao-login' no código do site e clique nele".
...
OBS: Um **flaky test** é um teste automatizado que *apresenta resultados inconsistentes*, passando em algumas execuções e falhando em outras, mesmo *na mesma versão do código e ambiente*. 
